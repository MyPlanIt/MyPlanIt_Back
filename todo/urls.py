...
urlpatterns = [
    ...
    path('todos/plan/<int:plan_id>/<int:plan_todo_id>/check', views.CheckPlanTodoView.as_view()),  # 플랜 투두 완료 기능
    path('todos/plan/<int:plan_todo_id>/delay', views.DelayPlanTodoView.as_view()),  # 플랜 투두 미루기 기능
    path('todos/plan/<int:plan_id>/todos', views.PlanTodosView.as_view()),  # 플랜 클릭 시 전체 투두 조회 기능
    path('todos/plan/<int:plan_todo_id>/detail', views.DetailTodoView.as_view()),  # 투두 세부 정보 조회 기능
